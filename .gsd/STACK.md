...
| `@tauri-apps/api` | `^2` | Tauri IPC interop client |
| `@tauri-apps/plugin-dialog` | `^2.7.2` | Native OS file & directory picker dialogs |
| `@tauri-apps/plugin-opener` | `^2` | Native OS file explorer & default app launcher |
...
| `tauri-plugin-opener` | `2` | Open files/URLs in default OS application |
| `tauri-plugin-dialog` | `2` | Native file and folder dialogs |
...
| `hex` | `0.4` | Hexadecimal byte encoding & decoding |
| `thiserror` | `1.0` | Strongly typed Rust error handling |
| `tracing` / `tracing-subscriber` / `tracing-appender` | `0.1` / `0.3` / `0.2` | Structured log telemetry and file logging |
...
| `globals` | `^17.9.0` | Global identifiers definition for ESLint |
| `@eslint/js` | `^10.0.1` | ESLint JavaScript recommended configuration |
| `@tauri-apps/cli` | `^2` | Tauri CLI development tools |
| `@vitejs/plugin-react` | `^4.6.0` | Vite React fast-refresh plugin |
...
|------|---------|----------|
| `package.json` | Node scripts & dependencies | Project Root |
...
| `vite.config.ts` | Vite bundler & server configuration | Project Root |
| `vitest.config.ts` | Vitest testing setup & jsdom environment | Project Root |
| `tsconfig.json` | TypeScript compiler configuration | Project Root |
| `model_capabilities.yaml` | Static model profiles & scale definitions | Project Root |

## Outdated Packages

| Package | Current | Latest | Risk Level |
|---------|---------|--------|------------|
| `@testing-library/jest-dom` | `7.0.0` | `7.0.1` | Very Low (Patch bump) |
| `@typescript-eslint/eslint-plugin` | `8.66.0` | `8.67.0` | Very Low (Minor patch bump) |
| `@typescript-eslint/parser` | `8.66.0` | `8.67.0` | Very Low (Minor patch bump) |
| `globals` | `17.9.0` | `17.11.0` | Very Low (Minor patch bump) |
| `@vitejs/plugin-react` | `4.7.0` | `6.0.5` | Low (Major version bump requires testing Vite compatibility) |
| `framer-motion` | `12.43.0` | `13.1.0` | Low (Major animation library version bump) |
