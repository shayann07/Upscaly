...
- [ ] **Process Safety**: Windows Job Object sidecar tree auto-kill (`JobObjectSetInformation`)
- [ ] **Instant Launch**: Cached Vulkan GPU discovery with 5s async timeout & driver hash validation
- [ ] **Visual Theme**: 60fps Ambient Liquid Shader (`Cosmic` `#23212C` + `Violet` `#36255C` + `Lavender` `#D2C3F6`)
- [ ] **Interactive Dropzone**: Atropos 3D Parallax tilt card with floating format tags (`PNG`, `JPG`, `MP4`) & ripple drop
- [ ] **Primary CTA**: Liquid Shimmer Pill CTA (`Vanilla` `#F1FEC8` → `Lavender` `#D2C3F6`) with 30px magnetic cursor pull
- [ ] **Hardware Slider**: 60fps clip-path Before/After comparison slider with 100%/200%/400% hardware zoom lens controls
- [ ] **Video Pipeline**: 3-Phase FFmpeg video pipeline with Rust RAII temp folder cleanup
- [ ] **Model Downloads**: Resumable HTTP Range downloader from GitHub Releases API with SHA256 validation
- [ ] **Notifications**: Floating Liquid Toast Stack with Framer Motion spring physics & one-click auto-fix CTAs
- [ ] **Audio & Settings**: Apple-like UI sound effects & persistent settings store (`app_data_dir/settings.json`)
