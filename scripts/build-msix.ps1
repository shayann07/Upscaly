...
foreach ($a in 'StoreLogo.png', 'Square44x44Logo.png', 'Square71x71Logo.png',
                'Square150x150Logo.png', 'Square310x310Logo.png',
                'Wide310x150Logo.png') {
    $p = Join-Path $icons $a
    if (-not (Test-Path $p)) { Fail "Missing asset $a. Regenerate with: npm run tauri icon" }
    Copy-Item $p (Join-Path $stage 'Assets')
}
