...
pub fn get_available_disk_space(dir: &Path) -> Result<u64, String> {
    #[cfg(windows)]
    {
        ...
        let wide_path: Vec<u16> = parent_dir.as_os_str().encode_wide().chain(std::iter::once(0)).collect();
...
pub fn verify_signature(data: &[u8], signature_hex: &str, public_key_hex: &str) -> Result<(), String> {
...
pub fn copy_bundled_models(app: &AppHandle, dest_dir: &Path) {
    ...
    if let Ok(path) = app.path().resolve("models", tauri::path::BaseDirectory::Resource) {
        candidate_dirs.push(path);
    }
    if let Ok(path) = app.path().resolve("src-tauri/models", tauri::path::BaseDirectory::Resource) {
        candidate_dirs.push(path);
    }
