...
export function UpdateBadge({ version, notes, phase, progress, onInstall }: UpdateBadgeProps) {
  ...
  return (
    <>
      ...
      {/*
        Portalled to <body>, and that is load-bearing. This badge lives
        inside the titlebar pill, which scales on hover -- and an ancestor
        with a transform becomes the containing block for fixed-position
        descendants. Rendered in place, this "fullscreen" wrapper collapsed
        into the pill's own box whenever the pointer crossed it: the dialog
        sat clipped in the titlebar corner and teleported between there and
        centre as the hover transition ran. No z-index fixes that; only not
        having a transformed ancestor does.
      */}
      {createPortal(
        <AnimatePresence>
          {isOpen && (
