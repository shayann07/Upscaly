...
interface ModelCardProps {
  model: ModelInfo;
  isInstalled: boolean;
  dlPct?: number | undefined;
  accentColor: string;
  onDownloadModel: (modelId: string) => void;
}

function ModelLiquidProgress({ pctVal }: { pctVal: number }) {
  return (
    <div
...
function ModelActionControl({
  ...
  if (isInstalled && !model.isCorrupt && !model.hasUpdate && !isDl) {
    return (
...
function ModelCatalogCard({
  ...
  return (
    ...
    >
      {isDl && <ModelLiquidProgress pctVal={pctVal} />}
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-2 space-y-1.5">
        {modelsList.map((m: ModelInfo) => (
          <ModelCatalogCard
            key={m.id}
            model={m}
            isInstalled={activeInstalled.includes(m.id)}
            dlPct={downloadingModels[m.id]}
            accentColor={accentColor}
            onDownloadModel={onDownloadModel}
          />
        ))}
